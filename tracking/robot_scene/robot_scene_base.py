...
from tracking.robot_scene.collision_torque_limit_prevention import ObstacleWrapperSim
...
class RobotSceneBase(object):
    ...
    # Armar 4
    MAX_ACCELERATION_ARMAR4_LEG = [3.0, 3.0, 3.0, 3.0, 3.0, 3.0]  # 6 joints
    ...
    def __init__(self,
        ...
        if robot_scene == 6 or robot_scene == 7:
            self._num_robots = 2
            plane_z_offset = 0
            if robot_scene == 6:
                ...
                # robot_scene == 7
                self._robot_name = "armar4_fixed_hands"
                robot_urdf = "armar4_fixed_hands"
                self._spline_joint_mask = np.array([False] * 12 + [True] * 18)
        ...
        for i in range(self._num_clients):
            fixed_base = True if i == self._obstacle_client_id or not self._floating_robot_base else False
            if self._no_self_collision:
                self._robot_id = p.loadURDF(urdf_path, useFixedBase=fixed_base,
                                            physicsClientId=i)
            else:
                self._robot_id = p.loadURDF(urdf_path, useFixedBase=fixed_base,
        ...
        if self._robot_name.startswith("armar4"):
            if "fixed_hands_and_legs" in self._robot_name:
                ...
                self._initial_max_accelerations = np.array(self.MAX_ACCELERATION_ARMAR4_LEG * self._num_robots +
                                                           ...
                                                           self.MAX_ACCELERATION_ARMAR4_ARM * self._num_robots)
                self._initial_max_jerk = np.array(self.MAX_JERK_ARMAR4_LEG * self._num_robots +
    ...
    @property
    def robot_base_balancing_mode(self):
        return self._robot_base_balancing_mode
    ...
    def _deactivate_self_collision_for_adjoining_links(self):
        ...
        if self._robot_name == "iiwa7":
            ...
            deactivate_self_collision_detection_link_name_pair_list_per_robot = [["leg_knee", "leg_ank2"],
                                                                                 ...
                                                                                 ["torso_yaw", "leg_hip1"]]
            if self._robot_name == "armar4_fixed_hands":
                deactivate_self_collision_detection_link_name_pair_list_per_robot += [["leg_hip1", "leg_hip3"]]
